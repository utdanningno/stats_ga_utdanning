...
setwd("C:/Users/torm/Documents/GitHub/stats_ga_utdanning")


ga_pw_utdanningno_data <- readRDS("data/ga_clean_pw_utdanningno_data_13_21.rds")
...
# ga_pw_utdanningno_data <- as_tibble(read_delim("./data/ga_pw_utdanningno_data_13_21.tsv", "\t", 
#                                                na = "NULL",
#                                                escape_double = FALSE, 
#                                                col_types = cols(
#                                                  year = col_integer(),
...
# ga_pw_utdanningno_data <- left_join(ga_pw_utdanningno_data, emp, by= "pageTitle")
# 
...
df.21 <- ga_pw_utdanningno_data %>%
  ...
  select(pageTitle, year, page_views_pst, pageviews) %>%
  arrange(desc(page_views_pst))

df.13 <- ga_pw_utdanningno_data %>%
  filter(page_views_pst < 1, year == 2013, page_type == "yrke") %>%
  slice_max(page_views_pst, n=9) %>%
  select(pageTitle, year, page_views_pst, pageviews) %>%
  arrange(desc(page_views_pst))

#### 2013

(top10.13 <- df.13$pageTitle)

#### 2021

(top10.21 <- df.21$pageTitle)
...
df.1 <- ga_pw_utdanningno_data %>%
  ...
  select(pageTitle, year, page_views_pst, pageviews, rank_ansatte, rank_ansatte_gruppe) %>%
  arrange(desc(page_views_pst))

df.2 <- df.1[df.1$pageTitle %in% top10.21,]

# df.2 %>%
#   select(pageTitle, year, rank_ansatte, rank_ansatte_gruppe) %>%
#   filter(year == 2021) %>%
#   rename(Yrke = pageTitle)

a <- ggplot(df.2, aes(x=as.character(year), y=page_views_pst*100, group= pageTitle, text= paste("Yrke:", pageTitle))) 
a <- a + geom_line(aes(color= pageTitle, label=rank_ansatte_gruppe, label2= rank_ansatte), size= 1) + xlab("År") + ylab("Prosentandel av sidevisninger") + labs(color= "Yrke", title="Utvikling til de mest besøkte yrkesbeskrivelsene i 2021") +
...
df.3 <- df.1[df.1$pageTitle %in% top10.13,]
...
b <- ggplot(df.3, aes(x=as.character(year), y=page_views_pst*100, group= pageTitle, text= paste("Yrke:", pageTitle))) 
b <- b + geom_line(aes(color= pageTitle, label=rank_ansatte_gruppe, label2= rank_ansatte), size= 1) + xlab("År") + ylab("Prosentandel av sidevisninger") + 
...
#   geom_text(aes(label=pageviews), vjust=1.6, color="white", size=3.5) +
...
freq <- data.frame(table(df.4$pageTitle)) #number of occurences 
yrke.2years <- as.character(freq$Var1[freq$Freq == 2]) #finding ocupations with both years

df.5 <- df.4[df.4$pageTitle %in% yrke.2years,] #excluding ocupations with only one occurence
# df.5 <- df.5[order(df.5$pageTitle),]

df.5.13 <- df.5[df.5$year == 2013,] #object for 2013
#df.5.13 <- df.5.13[order(df.5.13$pageTitle),]
df.5.13 <- arrange(df.5.13, pageTitle)


df.5.21 <- df.5[df.5$year == 2021,] #object for 2021
df.5.21 <- arrange(df.5.21, pageTitle)

#test that both years are sorted correct
all.equal(df.5.13[,1], df.5.21[,1]) #should be true

#new df with difference 2021 and 2013
diff <- data.frame(df.5.13$pageTitle, df.5.13$page_views_pst, df.5.21$page_views_pst, df.5.21$page_views_pst-df.5.13$page_views_pst) 
names(diff) <- c("pageTitle", "2013", "2021", "increase_page_views_pst")
...
diff.2 <- left_join(diff.2, emp, by="pageTitle") # joining with employment data
...
diff.2 <-diff.2 %>%
  ...
  rename(yrke=pageTitle, endring_pst=increase_page_views_pst, rank_gruppe=rank_ansatt_gruppe) %>%
...
diff.long <- left_join(diff.long, emp, by="pageTitle")


c <- ggplot(diff.long, aes(x=as.character(year), y=page_views_pst*100, group= pageTitle, text= paste("Yrke:", pageTitle))) 
c <-   c + geom_line(aes(color= pageTitle, label= rank_ansatte_gruppe, label2= rank_ansatte), size= 1) + xlab("År") + ylab("Prosentandel av sidevisninger") + labs(color= "Yrke", title = "Endring i prosentandel sidevisinger til de yrkene m størst endring fra 2013 til 2021") +
  ggthemes::theme_economist()
...
dfu.13 <- ga_pw_utdanningno_data %>%
  filter(page_views_pst < 1, year == 2013, page_type == "utdanningsbeskrivelse", pageTitle!="") %>%
  slice_max(page_views_pst, n=10) %>%
  select(pageTitle, year, page_views_pst, pageviews) %>%
  arrange(desc(page_views_pst))

(top10.13u <- dfu.13$pageTitle)
top10.13u <- na.omit(top10.13u)
(top10.21u <- dfu.21$pageTitle)
...
dfu.2 <- dfu.1[dfu.1$pageTitle %in% top10.21u,]

e <- ggplot(dfu.2, aes(x=as.character(year), y=page_views_pst*100, group= pageTitle, text= paste("Utdanning:", pageTitle))) 

e <- e + geom_line(aes(color= pageTitle), size= 1) + xlab("År") + ylab("Prosentandel av sidevisninger") + labs(color= "Utdanningsbeskrivelse", title="Utvikling til de mest besøkte Utdanningsbeskrivelsene i 2021") + ggthemes::theme_economist()
...
dfu.3 <- dfu.1[dfu.1$pageTitle %in% top10.13u,]
f <- ggplot(dfu.3, aes(x=as.character(year), y=page_views_pst*100, group= pageTitle, text= paste("Utdanning:", pageTitle))) 

f <- f + geom_line(aes(color= pageTitle), size= 1) + xlab("År") + ylab("Prosentandel av sidevisninger") + labs(color= "Utdanningsbeskrivelse", title="Utvikling til de mest besøkte Utdanningsbeskrivelsene i 2013") +
  ggthemes::theme_economist()
...
freq.u <- data.frame(table(dfu.4$pageTitle)) #number of occurences 
...
dfu.5 <- dfu.4[dfu.4$pageTitle %in% ut.2years,] #excluding ocupations with more or less occurences


dfu.5.13 <- dfu.5[dfu.5$year == 2013,] #object for 2013
dfu.5.13 <- arrange(dfu.5.13, pageTitle)


dfu.5.21 <- dfu.5[dfu.5$year == 2021,] #object for 2021
out <- data.frame(table(dfu.5.21$pageTitle)) #Helseadminstrasjon comes twice in 2021
out <- as.character(out$Var1[out$Freq != 1])
dfu.5.21 <- dfu.5.21[! dfu.5.21$pageTitle %in% out,]    

dfu.5.21 <- arrange(dfu.5.21, pageTitle)
...
#new df with difference 2021 and 2013
diff.u <- data.frame(dfu.5.13$pageTitle, dfu.5.13$page_views_pst, dfu.5.21$page_views_pst, dfu.5.21$page_views_pst-dfu.5.13$page_views_pst) 
names(diff.u) <- c("pageTitle", "page_views_pst_13", "page_views_pst_21", "increase_page_views_pst")
...
diff.u2 <- diff.u2 %>%
  rename(Utdanning=pageTitle, Endring_pst=increase_page_views_pst, '2013'=page_views_pst_13, '2021'=page_views_pst_21)
